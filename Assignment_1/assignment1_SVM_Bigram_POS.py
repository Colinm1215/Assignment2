...
def bigram_token(text, pos_filter={'NN', 'JJ'}):
    tokens = word_tokenize(text.lower())
    pos_tags = pos_tag(tokens)
    filtered_words = [
        (word, tag) for word, tag in pos_tags
        if word.isalpha() and word not in stop_words and tag in pos_filter
    ]
...
fake = 'fake.csv'
real = 'true.csv'
processed_data_path = 'processed_data_4.csv'
csvs = [fake, real]
